...
export default function AuthViewModel({
  ...
  async function loginWithEmailAndPassword(email, password) {
    const { result, error } = await LoginWithEmailAndPasswordUseCase.execute(
      email,
      password,
    );
    ...
    if (error) {
      ...
      switch (error.code) {
        ...
        case "auth/too-many-requests":
          setError({
            password: "Too many unsuccesful attempts. Please try again later.",
          });
          break;
        default:
  ...
  async function signUpWithEmailAndPassword(email, password) {
    const { result, error } = await SignUpWithEmailAndPasswordUseCase.execute(
      email,
      password,
    );
